import React from "react";
import { motion, useScroll } from "framer-motion";
import { useRef } from "react";
import LiIcon from "./LiIcon";

const Details = ({ type, time, place, info }) => {
  const ref = useRef(null);
  return (
    <li
      ref={ref}
      className="my-8 first:mt-0 last:mb-0 w-[60%] mx-auto flex flex-col items-center justify-between max-md:w-[80%]"
    >
      <LiIcon reference={ref} />
      <motion.div
        initial={{ y: 50 }}
        whileInView={{ y: 0 }}
        transition={{ duration: 1, type: "spring" }}
      >
        <h3 className="capitalize font-bold text-2xl max-sm:text-xl max-[479px]:text-lg ">
          {type}
        </h3>
        <span className="capitalize font-medium text-black/75 max-[479px]:text-sm">
          {time} | {place}
        </span>
        <p className="font-medium w-full max-md:text-sm">{info}</p>
      </motion.div>
    </li>
  );
};

const Education = () => {
  const ref = useRef(null);
  const { scrollYProgress } = useScroll({
    target: ref,
    offset: ["start end", "center start"],
  });
  return (
    <div className="my-64">
      <h2 className="font-bold text-8xl mb-32 w-full text-center max-md:text-6xl max-[479px]:text-4xl max-md:mb-16">
        Education
      </h2>
      <div
        ref={ref}
        className="w-[75%] mx-auto relative max-lg:w-[90%] max-md:w-full"
      >
        <motion.div
          style={{ scaleY: scrollYProgress }}
          className="absolute left-9 top-0 w-[4px] h-full bg-black origin-top max-md:w-[2px] max-md:left-[30px] max-[479px]:left-[20px]"
        />
        <ul className="w-full flex flex-col items-start justify-between ml-4 max-[479px]:ml-2">


          <Details
            type="Secondary Education (ICSE)"
            time="2010 - 2020"
            place="St. Joseph’s Secondary School, Visakhapatnam"
            info="Scored 90% in board exams. Built strong foundational knowledge in science, mathematics, and English."
          />
          <Details
            type="Higher Secondary (CBSE)"
            time="2020 - 2022"
            place="Kendriya Vidyalaya Sangathan, Visakhapatnam"
            info="Scored 85% overall. Studied core science subjects with a focus on Mathematics, Physics, and Computer Science."
          />
          <Details
            type="B.Tech in Computer Science and Engineering"
            time="2022 - 2026 (Expected)"
            place="ITER, SOA University, Bhubaneswar"
            info="CGPA: 8.66 (Till 5th Semester). Relevant courses include Data Structures, Web Development, DBMS, Operating Systems and Computer Networking."
          />
        </ul>
      </div>
    </div>
  );
};

export default Education;
